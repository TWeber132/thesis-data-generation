...
def pick(robot, pose0, pose1):
    print(pose0, pose1)
    pos0 = np.float32((pose0[0][0], pose0[0][1], pose0[0][2]))
    up_pos0 = np.float32((pose0[0][0], pose0[0][1], 1.8))
    pos1 = np.float32((pose1[0][0], pose1[0][1], pose1[0][2]))
    up_pos1 = np.float32((pose1[0][0], pose1[0][1], 1.8))
    rot = np.float32((1.0, 1.0, 0.0, 0.0))

    timeout = robot.movep((up_pos0, rot), speed=0.001)
    timeout |= robot.movep((pos0, rot), speed=0.001)
    timeout |= robot.ee.grasp()
    timeout |= robot.movep((up_pos1, rot), speed=0.001)
    timeout |= robot.movep((pos1, rot), speed=0.001)
