...
def sample_distribution(prob, n_samples=1):
    ...
    rand_ind = np.random.choice(
        np.arange(len(flat_prob)), n_samples, p=flat_prob, replace=True)
...
def sphAPR_to_recXYZ(azimuth, polar, radius):
    """Abstraction for converting from spherical coordinates to rectangular.

    Args:
        azimuth (float): the azimuth angle
        polar (float): the polar angle
        radius (float): the radius

    Returns:
        np.ndarray: an array of 3 float for x, y, and z coordinate
    """
...
def sphAPR_to_rotm(xyz, azimuth):
    """Function to get the orientation of a point on a sphere.

    The point will always face with it's z-axis inwards to the center of the sphere.

    Args:
        xyz (np.ndarray of three floats): the x, y, z coordinate of the point
        azimuth (float): the azimuth angle

    Returns:
        np.ndarray: 3x3 rotation matrix for the rotation of the point
    """
    z_axis = xyz
    ...
    rotm = np.stack([x_axis, y_axis, z_axis], axis=1)
    return rotm


def get_pose_on_sphere(azimuth, polar, radius, sph_pos):
    """Gets the pose of a point on a sphere given in spherical coordinates

    Args:
        azimuth (float): the azimuth angle
        polar (float): the polar angle
        radius (float): the radius
        sph_pos (tuple): the pose of the sphere in space

    Returns:
        tuple: the pose of the point 
    """
    xyz = sphAPR_to_recXYZ(azimuth, polar, radius)
    pos = xyz + sph_pos
...
def get_matrix(pos, quat):
    ...
    # rearrange xyzw to wxyz
    quat = quat[[3, 0, 1, 2]]
    rot_m = quaternions.quat2mat(quat)
    matrix[:3, 3] = pos
    matrix[:3, :3] = rot_m
    return matrix
...
def apply_transform(transform_to_from, points_from):
    """Transforms points (3D) into new frame.
...
class ImageRotator:
    ...
    def __call__(self, x_list, pivot, reverse=False):
        ...
        for i, angle in enumerate(self.angles):
            ...
            # in degrees
            alpha: float = angle if not reverse else (-1.0 * angle)
            ...
            M: torch.tensor = kornia.get_rotation_matrix2d(
                center, angle, scale)
